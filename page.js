define( "ModalPage" , [ "Base" , "Template" , "RequireFile" , "ModalView" ] , function( Base , Template , RequireFile , ModalView ){
    ...
    var PageList    = Base.extend( function( pageListId , $pageContainer , opt ){
        ...
        addBeforeDisplayEvent   : function( funcs ){
            funcs   = funcs instanceof Array ? funcs : [ funcs ];
            this._pageListConfig.originOpt.beforeDisplayEvents = this._pageListConfig.originOpt.beforeDisplayEvents.concat( funcs );
            return this;
        } ,
        addAfterDisplayEvent    : function( funcs ){
            funcs   = funcs instanceof Array ? funcs : [ funcs ];
            this._pageListConfig.originOpt.afterDisplayEvents = this._pageListConfig.originOpt.afterDisplayEvents.concat( funcs );
            return this;
        } ,
        handleDisplayEvent  : function( pageName , type ){
            var _events     = this._pageListConfig.originOpt[ type + "Events" ];
            if( _events ){
                for( var i = 0 , len = _events.length; i < len; i++ ){
                    if( typeof _events[ i ] === "function" ){
                        if( _events[ i ].apply( this , [ pageName ] ) === false ){
                            break;
                        }
                    }
                }
        ...
        urlHistoryBack  : function( backNum ){
            var _config     = this._pageListConfig;
            backNum = _config.urlHistory.length - ( backNum || 1 );
            if( backNum >= 0) {
                this[ _config.urlHistory[ backNum - 1 ] || this._pageListConfig.originOpt.defaultHistroyPage ].display( function(){
                    _config.urlHistory = _config.urlHistory.slice( 0 , backNum );
                } );
            }
            return this;
        } 
        ...
        Page    = Base.extend( function( belongPageList , jsPath , pageName ){
            ...
            displayPageModal    : function( func ){
                ...
                _self._pageConfig.belongPageList.handleDisplayEvent( _self._pageConfig.pageName , "beforeDisplay" );
                _self.getPageModal( function(){
                    ...
                    _self._pageConfig.belongPageList.handleDisplayEvent( _self._pageConfig.pageName , "afterDisplay" );
                } );
                return this;
            } ,
            ...
            getPageModal        : function( func ){
                this._pageConfig.belongPageList._pageListConfig.urlHistory.push( this._pageConfig.pageName );
